...
function recordReplayBridgeSource({ childProcessVar, fsVar, pathVar }) {
  return [
    ...
    `"linux-record-replay-import-skill":async({source:e,target:t,targetDir:n,mode:r,dryRun:a,allowUnsupported:o}={})=>{let s=codexLinuxRecordReplayString(e);if(!s)return{ok:!1,action:"skill.import",message:"source is required"};let i=["skill","import","--source",s];if(t)i.push("--target",String(t));if(n)i.push("--target-dir",String(n));if(r)i.push("--mode",String(r));a&&i.push("--dry-run");o&&i.push("--allow-unsupported");return codexLinuxRecordReplayRun(i,30000)}`,
...
function codexLinuxRecordReplayWriteTempJson(e){let t=process.env.XDG_RUNTIME_DIR||process.env.TMPDIR||"/tmp",n=${pathVar}.join(t,"codex-record-replay-traces");${fsVar}.mkdirSync(n,{recursive:true});let r=${pathVar}.join(n,"trace-"+Date.now()+"-"+process.pid+"-"+Math.random().toString(36).slice(2)+".json");${fsVar}.writeFileSync(r,String(e));return r}
